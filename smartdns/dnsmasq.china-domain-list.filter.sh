...
# https://github.com/imyip/dnsmasq-china-list-for-smartdns
...
OUTPUT_FILE="china.domains.list"

# download the list
curl -fkLo "${DOMAINS_LIST_FILE}" "${DOMAINS_LIST_URL}"

# filter length in 3 to 8 words and '.com' domain
rg -Ne 'domain-rules /\S{3,11}\.(com|net|org|co)' "${DOMAINS_LIST_FILE}" |\
    ...
    # rg -Nv 'domain-rules /(\d\S*|[^-]*-\S*|\w{1,5}\d*|\w\d\w*)\.(com|net|org|co)' |\
	# save only domains
	awk '{print $2}' | cut -d'/' -f2 > "${OUTPUT_FILE}"
